  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Loyalty request failed: ${response.status}`,
          );
        }
    ...
    return (
      <section className="rounded-2xl border border-wire/70 bg-vault/15 px-5 py-12 text-center">
        <p className="text-sm text-paper/65">
          Connect your wallet to view Loyalty.
        </p>
      </section>
    );
  }
    ...
    <section className="space-y-3">
      ...
      <div className="rounded-2xl border border-wire/70 bg-vault/15 p-4">
        ...
        <p className="mt-4 text-[9px] leading-relaxed text-paper/25">
          Loyalty is derived from verified Settlement Certificates issued after successful Rekber settlement.
        </p>
      </div>
