  ...
  async run(input) {
    ...
    for (let iteration = 0; iteration < 4; iteration += 1) {
      const response = await client.chat.completions.create({
        model,
        messages,
        tools: toolDefinitionsForSkill(input.skill) as any,
        tool_choice: "auto",
        temperature: 0.2,
      });
      ...
      if (!message.tool_calls?.length) {
        return {
          answer: message.content || "No recommendation available.",
          dealStage,
          proposal,
          provider: "groq",
          model,
    ...
    throw new Error("Groq reached maximum tool iterations.");
  },
};
