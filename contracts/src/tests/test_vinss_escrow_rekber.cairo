// Payment can leave custody only when the
...
fn fund_default_custody(
    ...
    let calldata = default_deposit_calldata(token_address);
...
fn release_requires_both_keys_and_returns_full_principal() {
    ...
    assert(rekber.get_reserved_amount(token_address) == 0, 'reserve remains');
    assert(
        token.allowance(rekber_address, privacy_pool()) == PRINCIPAL.into(),
        'bad output allowance',
    );
}
